...
const DynamicMap = dynamic(() => import('@/components/ComplianceMap'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full bg-[#030811]/90 animate-pulse flex flex-col items-center justify-center border border-white/5 rounded-2xl relative overflow-hidden">
      <div className="text-purple-400 font-mono text-sm tracking-widest uppercase mb-2 animate-pulse">Initializing Compliance Command Grid...</div>
      <div className="text-[10px] text-white/40 uppercase tracking-widest font-mono">calculating civilian clearances & sector resistances</div>
      ...
      <div className="absolute bottom-0 left-0 w-4 h-4 border-b border-l border-purple-500/30" />
      <div className="absolute bottom-0 right-0 w-4 h-4 border-b border-r border-purple-500/30" />
    </div>
  )
});

export default function ComplianceMapContainer(props: any) {
  return (
